...
public class SurveyService {
    public List<Survey> list() throws SQLException {
        ...
        return DBUtil.selectMore(
                "select sid sid, u.uid, title, brief, cover_url, nickname from survey s, users u where s.uid = u.uid and status = '已发布' order by sid desc",
                values,
                r -> {
    ...
    public Survey get(int sid) throws SQLException {
        ...
        DBUtil.selectOne(
                "SELECT sid, title, brief, cover_url, thanks FROM survey where sid = ?",
                values,
                resultSet -> {
                    survey.sid = sid;
    ...
    public void save(Survey survey) throws SQLException {
        ...
        DBUtil.updateOrInsertOrDelete("UPDATE survey SET title = ?, brief = ?, cover_url = ?, thanks = ? WHERE sid = ?", values1);
    ...
    public int insert() throws SQLException {
        try (Connection c = DBUtil.getConnection()) {
            String sql = "INSERT INTO survey (uid, title, brief, cover_url, thanks, status) VALUES (1, '请输入调查问卷题目', '请给与简单介绍', '/img/cover.png', '感谢参与', '已发布')";
